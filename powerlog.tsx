...
// Helper to format hours nicely (e.g., 14:00)
const formatTimeStr = (dateObj) => {
  return dateObj.toTimeString().split(' ')[0].substring(0, 5);
};

// Generate initial mock logs for the last 3 days to populate charts beautifully
...
// Generate realistic utility daily reports reflecting logs + base loads
const generateMockUtilityData = (logs, targetDaysCount = 3) => {
  ...
  for (let i = targetDaysCount - 1; i >= 0; i--) {
    ...
    // Base load is background phantom draw (0.25 kWh per hour, roughly 6.0 kWh/day)
    // plus a random fluctuation
...
export default function App() {
  ...
  const [utilityData, setUtilityData] = useState(() => {
    ...
    // Otherwise build mock data from initial logs
    const initialLogs = generateMockLogs(DEFAULT_APPLIANCES);
    return generateMockUtilityData(initialLogs, 3);
  });

  // Navigation tab state
  const [activeTab, setActiveTab] = useState('dashboard'); // dashboard | logs | appliances | utility

  // UI Interactive States
  const [activeTimers, setActiveTimers] = useState({}); // { applianceId: startTimeTimestamp }
  ...
  const handleStopTimer = (applianceId) => {
    ...
    const durationMins = Math.max(1, Math.round(durationMs / 60000)); // Minimum 1 minute
  ...
  const handleAddUtilityEntry = (e) => {
    ...
    // Filter out existing on same date
  ...
  const handleCsvImport = (e) => {
    ...
    lines.forEach(line => {
      ...
      if (cols.length >= 2) {
        ...
        // Simple validation of Date (YYYY-MM-DD) and numeric kWh
  ...
  // Quick generate demo utility data matching logs + baseline
  const handleAutoGenerateUtility = () => {
    ...
    const generated = loggedDates.map(dateStr => {
      ...
      // Standard continuous household phantom/standby background: ~6.2 kWh/day
    ...
    // Merge with priority over duplicates
    const finalData = [...utilityData];
    generated.forEach(g => {
  ...
  // Aggregate stats per day comparing utility with logged
  const dateWiseComparison = useMemo(() => {
    // Collect all dates from both logs and utility
    ...
    return Array.from(allDates)
      ...
      .map(date => {
        ...
        // Percent of total utility accounted for by our logs
  ...
  const totals = useMemo(() => {
    ...
    // Only sum utility days where we also have logged data or all utility days
  ...
  // Aggregate energy breakdown by appliance
  ...
  // Current active timer tracking helper
                        ...
                        <svg viewBox="0 0 500 240" className="w-full h-auto">
                          ...
                          {(() => {
                            const maxVal = Math.max(
                              ...
                              10 // floor baseline scale
                            ) * 1.1; // padding

                            const chartHeight = 180; // height inside the 200 grid
                            const renderDays = [...dateWiseComparison].slice(0, 7).reverse(); // limit 7 and show oldest-to-newest
                            const barGap = 60;
                            const barWidth = 14;
                          ...
                          <div>
                            {isTimerRunning ? (
                              <button
                                ...
                                className="px-3 py-1.5 text-xs font-bold bg-rose-650 hover:bg-rose-600 text-slate-100 rounded-lg flex items-center gap-1 transition"
                              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fadeIn">
            ...
            <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {appliances.map(app => {
                  // Count total logged runs for this appliance to show popularity
                ...
                <form onSubmit={handleAddUtilityEntry} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-emerald-500 font-mono"
                    />
                    <p className="text-[10px] text-slate-500 mt-1">Found on your utility company dashboard (daily breakdown / hour summary).</p>
                  </div>
